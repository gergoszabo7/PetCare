...
export class NewPetDialogComponent {
  ...
  private filterBreeds(value: string | Breed): Breed[] {
    ...
    return this.breeds.filter(breed => breed.viewValue.toLowerCase().includes(filterValue));
  }
  ...
  search(value: string) { 
    ...
    return this.breeds.filter(option => option.value.toLowerCase().includes(filter));
  }
}
